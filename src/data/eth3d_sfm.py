...
from .eth3d_base import indoor_val_split, indoor_test_split, outdoor_val_split, outdoor_test_split

val_split = indoor_val_split + outdoor_val_split
test_split = indoor_test_split + outdoor_test_split
...
# split_txt = '../data_json/marigold_data_split/eth3d/eth3d_courtyard.txt'
from pathlib import Path
dataset_folder = (Path(__file__).parent.parent.parent / "datasets").resolve()

class ETH3D_SfM(BaseDataset):
    def __init__(self, args, mode, scene='indoor'):
        super(ETH3D_SfM, self).__init__(args, mode)
        ...
        # if mode == 'val':
        #     scene_list = val_split
        # else:
        #     scene_list = test_split
    ...
    def __getitem__(self, idx):
        ...
        # dep_sp = self.get_sparse_depth(dep, self.args.num_sample, match_density=True)
        scene_name = rgb_rel_path.split('/')[1]
        dep_sp_path = os.path.join(data_dir, 'sparse_depth', scene_name, os.path.basename(rgb_rel_path).replace('.JPG', '.npy'))

        dep_sp = torch.tensor(np.load(dep_sp_path, allow_pickle=True)).unsqueeze(0).float()

        output = {'rgb': rgb, 'dep': dep_sp, 'gt': dep, 'K': K, 'pattern': PATTERN_IDS['sfm']}
